"""SES → SNS → Lambda で受け取ったメールをログに吐くだけの処理。

SES の SNS action は notification の content フィールドに生 MIME を載せる。
Receipt Rule 側で encoding=UTF-8 を指定しているので、content はそのまま文字列として
扱える前提 (Base64 にしている場合は base64.b64decode が必要)。
"""
